...
def api(request):
    ...
    if request.method == 'POST':
        ...
        if method != None:
            ...
            smtp.starttls()
            smtp.login('Ваша почта', 'Пароль для входа')
            code = random(10000, 90000)
            smtp.sendmail('Ваша почта', f'{email}', f'confirmation code for messenger {code}')
            smtp.quit()
        ...
        if method != None:
            if method == 'get_client_name':
                ...
                user_id = request.GET.gey('user_id')
                account = Account.objects.get(id=user_id)
                event = {'type': 'message_now', 'message': message, 'peer_id': user_id, 'first_name': account.first_name, 'last_name': account.last_name}
